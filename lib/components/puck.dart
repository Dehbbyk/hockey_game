...
class Puck extends SpriteComponent with HasGameRef<GlowHockeyGame>, CollisionCallbacks {
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (other is Paddle) {
      ...
      if (intersectionPoints.first.x <= radius || intersectionPoints.first.x >= gameRef.size.x - radius) {
        velocity.x = -velocity.x;
      } else if (intersectionPoints.first.y <= radius || intersectionPoints.first.y >= gameRef.size.y - radius) {
        velocity.y = -velocity.y;
      }
    }
  }
